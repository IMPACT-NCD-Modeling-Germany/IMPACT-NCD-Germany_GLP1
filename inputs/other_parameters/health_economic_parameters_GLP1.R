...
cost_combine <- data.table(NULL)
  ...
  price_index <- data.table( year   =    c(13:45),                               # Prize drop is only related to calendar year
...
write_fst(price_index, "./inputs/other_parameters/GLP_price_index.fst")
...
#############################################################################################
  ...
  cost_data[scenario == "sc3", 
            GLP_cost := fifelse(trtm_year == 0, 1844,
